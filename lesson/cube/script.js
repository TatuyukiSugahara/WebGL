...
onload = function()
{
    ...
    uniLocation[3] = gl.getUniformLocation(prg,'lightDir');
    uniLocation[4] = gl.getUniformLocation(prg, 'eyedir');
    uniLocation[5] = gl.getUniformLocation(prg, 'ambientcolor');
    uniLocation[6] = gl.getUniformLocation(prg, 'eyePosition');
    uniLocation[7] = gl.getUniformLocation(prg, 'cubeTexture');
    uniLocation[8] = gl.getUniformLocation(prg, 'reflection');
    ...
    //イメージ配列。
    var cuvesorce = new Array('cube_PX.png',
                              'cube_PY.png',
                              'cube_PZ.png',
                              'cube_NX.png',
                              'cube_NY.png',
    ...
    //テクスチャ作成。
    CreateCubeTexture(cuvesorce,cubeTarget);
        
    //カメラの座標。
    var eyePos = [0.0,0.0,20.0];
    ...
    (function(){
        ...
        gl.uniform3fv(uniLocation[3], lightDir);
        gl.uniform3fv(uniLocation[4], eyeDir);
        gl.uniform4fv(uniLocation[5], ambient);
        gl.uniform4fv(uniLocation[6], eyePos);
        gl.activeTexture(gl.TEXTURE0);
        gl.uniform4fv(uniLocation[7], 0);
        gl.uniform4fv(uniLocation[8], false);
        ...
        gl.uniform4fv(uniLocation[5], ambient);
        gl.uniform4fv(uniLocation[7], 0);
        gl.uniform4fv(uniLocation[8], true);
    ...
    //キューブマップテクスチャ作成。
    function CreateCubeTexture(source, target)
    {
        //配列。
        var img = new Array();
        ...
        {
            img[i]= new cubeMapImage();
            img[i].data.src = source[i];
        }
        ...
        {
            if(img[0].data.imageDataLoaded &&
                img[1].data.imageDataLoaded &&
                img[2].data.imageDataLoaded &&
                img[3].data.imageDataLoaded &&
                img[4].data.imageDataLoaded &&
                img[5].data.imageDataLoaded
            )
            {
                generateCubeMap();
            }
        ...
        function generateCubeMap()
        {
            ...
            {
                // テクスチャへイメージを適用
                gl.texImage2D(target[i], 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, cImg[i].data);
            }
